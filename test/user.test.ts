...
import { BserApiError } from "../src/index.ts";
import { MatchingMode, MatchingTeamMode } from "../src/constants.ts";
import { client, sharedState, hasApiKey } from "./helpers.ts";
...
test.skipIf(!hasApiKey)("getByNickname — 존재하지 않는 닉네임 BserApiError", async () => {
  await expect(client.user.getByNickname("___존재하지_않는_닉네임___")).rejects.toThrow(BserApiError);
});
